...
const CardWeb = () => { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:8080/car/insertDetail?userId=${userId}`, {
        name: formData.name,
      ...
      // ✅ Redirect user to their unique Thymeleaf page
      window.location.href = `http://localhost:8080/car/card/${userId}`;
    } catch (error) {
      console.error(error);
  ...
  return (
    ...
    >
      ...
      >
        User ID: {userId} <br />
        <small className="text-muted">Remember this ID</small>
      </div>
